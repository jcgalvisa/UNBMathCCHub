...
# ╔═╡ 3a2938c2-0f92-4fd4-9b2f-1dd090d90d01
using PlutoUI

# ╔═╡ 68e9248d-80e3-4f5d-a5f0-4553fa83e4b5
begin
	using LinearAlgebra
	using GraphPlot
...
# ╔═╡ 9ac66160-a826-11ee-3e7d-b105fd533541
...
# ╔═╡ e07af1fe-49bd-4d5f-968c-b75d1b851a2d
md"""Vamos a usar las siguientes librerías:"""

# ╔═╡ fb5e4951-8d8c-4729-ade2-dbe45a0c9e80
md"""# Introducción"""
...
# ╔═╡ 7151ef35-a292-4eea-90ed-906b46a41d37
md"""### Tipos de nodos"""

# ╔═╡ bee1bad1-780e-4ff7-96c3-44aaba8b6666
md"""Los nodos, representados visualmente como puntos o círculos en un grafo, son elementos esenciales en la estructura de este. La visualización puede mejorarse mediante el uso de colores y tamaños para destacar atributos específicos. Los nodos pueden ser cualquier objeto, matemático o no, proporcionando versatilidad en la modelización.
...
# ╔═╡ f4cbaeed-7bba-48e2-8a46-5210a14d7327
gplot(G₁, nodelabel=1:3)

# ╔═╡ 836f5b97-8a6b-4243-9421-5493599d3d9d
md"""El número de vértices del grafo anterior se obtiene de la siguiente manera"""
...
md"""También podemos construir un grafo de la siguiente forma"""

# ╔═╡ a2eb3574-2041-479d-87e9-eba7684044b7
begin
	Gr = Graph() #Se genera el grafo
	add_vertices!(Gr, 5) #Agregamos 5 vértices
	add_edge!(Gr, 1, 2) #Se conecta el nodo 1 con el 2
	add_edge!(Gr, 1, 3) #Se conecta el nodo 1 con 3
	add_edge!(Gr, 2, 4) #Se conecta el nodo 2 con 4
	add_edge!(Gr, 3, 4) #Se conecta el nodo 3 con 4
	add_edge!(Gr, 3, 5) #Se conecta el nodo 3 con 5
	add_edge!(Gr, 4, 5) #Se conecta el nodo 4 con 5
end
...
# ╔═╡ 016d02b0-1f1d-4125-a0fd-9d5b866698ee
md"""En Graphs.jl, agregar elementos a un grafo es siempre una operación segura en el sentido de que se conservan las etiquetas de vértices y bordes, observe"""

# ╔═╡ a87cb453-1097-48a0-b0b4-d2e49a5ca3b1
begin
	add_vertex!(Gr)
	add_edge!(Gr, 5, 6)
end

# ╔═╡ d088e422-35b1-4699-ab0a-b88eca559bd6
gplot(Gr, nodelabel=1:nv(Gr), edgelabel=1:ne(Gr))
...
# ╔═╡ 2926259f-6c28-4f88-845d-df917c7a66bc
gplot(Gr, nodelabel=1:nv(Gr), edgelabel=1:ne(Gr))

# ╔═╡ a911c56e-302f-4a28-9e39-5ccf469d9ae0
md"""También es posible construir un grafo con aristas aleatorias usando la siguiente función"""

# ╔═╡ 736a7799-4dab-4c66-8032-2d60abe16369
G₂ = Graph(3, 3)

# ╔═╡ 7b6380fb-7be7-4184-89e7-1e07a29f91ae
gplot(G₂, nodelabel=1:3)

# ╔═╡ 675476f5-b0f6-45a1-a477-83421674f222
md"""Observe que Graphs.jl verifica el número máximo de aristas para el número dado de vértices y arroja un error si no se respeta este máximo:"""

# ╔═╡ b32a83e6-dd6f-4b59-b1b6-c4158bb7dd69
G₃ = Graph(3, 4)

# ╔═╡ b68d703b-cae4-4dbb-8744-b9869edb943c
G₄ = Graph(4, 2)

# ╔═╡ 716326fb-1e65-4a97-af8f-e98c806dd828
gplot(G₄, nodelabel=1:4)

# ╔═╡ fd34e171-b92a-47f7-993b-e5eae25fd3a4
md"""Los grafos recién creados no están dirigidos, lo que significa que no hay sentido de dirección en los bordes del grafo. Para crear un grafo con aristas dirigidas, se puede usar la siguiente función"""

# ╔═╡ b3aa5c63-40a4-48b3-bc6c-8ae7ea271c97
G₅ = DiGraph(3,3)

# ╔═╡ 369899b5-6401-4ef8-8cbb-0268f7f916bd
gplot(G₅)

# ╔═╡ be4d7499-57b4-41da-8783-a2723d40dfb4
md"""# Matriz de adyacencia"""

# ╔═╡ 73f84cc7-7e8d-4806-b0d0-99ac54dd3a1c
md"""La matriz de adyacencia es una representación matricial de un grafo $G=(V, E)$ que proporciona información sobre las conexiones entre sus nodos $V=\{v_1, v_2, \cdots, v_n\}$. Esta matriz es cuadrada y de tamaño $n\times n$, donde $n$ es el número de nodos en el grafo. La entrada $a_{ij}$ de la matriz indica si existe una arista entre los nodos $i$ y $j$, de la siguiente forma

$a_{ij}=1, \text{ si } (v_i, v_j)\in E,$

$a_{ij}=0, \text{ si } (v_i, v_j)\notin E.$

La diagonal principal contiene siempre ceros en un grafo simple (sin bucles).
...
# ╔═╡ b300ef48-25af-477b-ac6b-c8d8014e75b4
...
# ╔═╡ af2c8a58-3283-4a84-aefc-b72f44f75d1f
md"""# Matriz de incidencia"""

# ╔═╡ 10784332-a958-43f8-8324-8f21b329034a
md"""La matriz de incidencia es otra representación matricial de un grafo $G=(V,E)$, con $V=\{v_1, v_2, \cdots, v_n\}$ y $E=\{e_1, e_2, \cdots, e_m\}$, pero a diferencia de la matriz de adyacencia, esta proporciona información sobre las conexiones entre nodos y aristas. Para un grafo con $n$ nodos y $m$ aristas, la matriz de incidencia $B$ es de tamaño $n×m$, tal que
...
# ╔═╡ fc928800-1e16-4446-bd3a-1acfbc20a465
G₈ = Graph(7, 5)

# ╔═╡ ddea61ca-c9ce-4baa-9334-eda8b1291575
gplot(G₈, nodelabel=1:7)

# ╔═╡ 34fd9429-2d36-4097-92fe-667ba05623f8
md"""Su matriz de incidencia es la siguiente"""

# ╔═╡ 635e0b5f-72a3-48b1-bf84-449b2442ba7e
incidence_matrix(G₈)

# ╔═╡ 76c2cd2d-fa8e-49bc-b387-1be4358f809e
md"""Veamos otro ejemplo, consideremos el siguiente grafo"""

# ╔═╡ 1f5ccf93-36af-422b-923b-6801f57ea76d
g₈ = Graph(4, 3)

# ╔═╡ 65105160-5ea4-41fa-b9d3-1bd7adb9eda2
gplot(g₈, nodelabel=1:4)

# ╔═╡ 5cf1dd13-f868-4683-ace9-15748b35ec76
md"""Su matriz de incidencia es:"""

# ╔═╡ 951a4471-b98a-47a5-90fb-d0b8fe9b0e3e
incidence_matrix(g₈)
...
# ╔═╡ 492827ee-02cb-4c90-a7f3-b27916f0cb0b
gplot(G₉)

# ╔═╡ 25a8edcf-b5d1-4116-9107-9ac8e85db2e6
md"""Grafo con forma de casa"""

# ╔═╡ 0ac9c5a5-07fd-4174-b9ea-e567f0e5f430
G₁₀ = smallgraph("house")

# ╔═╡ 9dde380e-013a-493a-b17a-4ba943aad79f
gplot(G₁₀)
...
# ╔═╡ ca45c20b-fdd9-4d67-b5d7-6f0b9e1d886a
gplot(G₁₁)

# ╔═╡ 99947d83-0568-416f-aeeb-20da3b3b3695
md"""# Referencias"""

# ╔═╡ 40ccd7c1-9832-4a8a-94b7-7b5a509bab42
md"""[1] Trudeau, R. (1994). Introduction to Graph Theory (2nd ed.). Dover Publications.

[2] Balakrishnan, V. K. (1991). Introductory Discrete Mathematics. Prentice Hall.
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "63213c3930750a9a82ae144b84abf0626c5d11fd"
