...
md"""**Teorema:**
Si $A$ es una matriz de tamaño $m \times n$ (con $m<n$) tal que $\text{rango } A = n$, entonces $A^TA$ es no singular y el sistema lineal $Ax = b$ tiene una única solución por mínimos cuadrados, dada por
...
begin
	...
	plot(x_i, y_i, seriestype=:scatter, label="datos")
end
...
# ╔═╡ d69326ee-76e9-4569-8216-223e47460240
begin
	plot(x_i, y_i, seriestype=:scatter, label="datos")
	xx = range(1, 3.5, length=100)
	polinomial(x) = x₂[1]*x.^2 .+ x₂[2]*x .+ x₂[3]
	plot!(xx, polinomial(xx), label="Ajuste")
end
