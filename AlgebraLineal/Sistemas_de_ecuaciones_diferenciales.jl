...
# ╔═╡ 9f24b753-50ad-4dec-85ce-7971a0492ca0
md"""# Introducción"""
...
md"""Finalmente mostramos la solcuión de la ecuación dada."""
...
$P^{-1}x'=P^{-1}Ax.$
Ahora bein, si $u=P^{-1}x,$ entonces $u'=P^{-1}x'$. Si sustituimos eso en la ecuación anterior se tiene que

$u'=(P^{-1}AP)(P^{-1}x)=Du.$

Este ya es un sistema giagonal que podemos resolver tal como lo mostramos anteriormente. Así
...
Y como $x=Pu$, entonces la solcuión general del sistema dado $x'=Ax$ es

$x(t)=Pu(t) = C_1Pu_1(t) + C_2Pu_2(t) + \ldots + C_nPu_n(t).$

Note que al realizar $Pu_i(t)$ con $0\leq i\leq n$ se obtienen los vectores propioss de $A$ asociados a $\lambda_i$, respectivamente.
...
# ╔═╡ 84df41c5-683b-4963-a61b-77d54ea287e0
md"""*Ejemplo:*
...
# ╔═╡ 2219c690-97eb-4bbc-a19e-558077534933
A₁ = [1 1; 4 1]
...
md"""Así, la solución general de la EDO es:"""

# ╔═╡ 7de540ca-2edd-4517-ad19-e448fc1065e8
println("x(t) = C₁ ", eigvecs(A₁)[:,1], " exp(", eigvals(A₁)[1], "t) + C₂ ",  eigvecs(A₁)[:,2], " exp(", eigvals(A₁)[2], "t)")

# ╔═╡ ef7984dc-581c-4ce8-bb74-b7cdef97a1ee
md"""En términos de componentes, esto se puede escribir como:"""
...
md"""Así, la solución general del sistema es
...
# ╔═╡ 6153cf5a-7f9c-4064-847a-56cf07776c5c
md"""Con esto ya conocemos la solcuión exacta del sistema, podemos crear funciones para cada solución:"""

# ╔═╡ a85b6a89-d541-40f3-a9ac-e98591ed860a
begin
	x₁(t) = C₁*P[1,1]*exp(t*λ[1]) + C₂*P[1,2]*exp(t*λ[2]) + C₃*P[1,3]*exp(t*λ[3]) #
	x₂(t) = C₁*P[2,1]*exp(t*λ[1]) + C₂*P[2,2]*exp(t*λ[2]) + C₃*P[2,3]*exp(t*λ[3])
	x₃(t) = C₁*P[3,1]*exp(t*λ[1]) + C₂*P[3,2]*exp(t*λ[2]) + C₃*P[3,3]*exp(t*λ[3])
end

# ╔═╡ d16d39f6-ed28-4482-88f5-1eb1cd55aec4
md"""Verifiquemos con las funciones anteriores las condiciones iniciales:"""
...
[2] Kolman, B., & Hill, D. R. (2006). Álgebra Lineal (8a ed.). Pearson.

[3] Grossman, S. I., & Flores Godoy, J. J. (2012). Álgebra Lineal (7a ed.). McGraw-Hill.

[4] Cullen, C. G. (1991). Linear Algebra and Differential Equations (2a ed.). Boston: PWS-Kent."""
