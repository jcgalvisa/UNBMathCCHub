...
let
	...
	x = [r * sin(φ) * cos(θ) for φ in phi_range, θ in theta_range]
	y = [r * sin(φ) * sin(θ) for φ in phi_range, θ in theta_range]
	z = [r * cos(φ) for φ in phi_range, θ in theta_range]

	# Graficar la esfera
	surface(x, y, z, xlabel="X", ylabel="Y", zlabel="Z", title="Esfera de Radio p = $r")
end
