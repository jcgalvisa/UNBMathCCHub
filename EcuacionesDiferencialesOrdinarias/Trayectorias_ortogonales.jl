...
begin
	using Plots, DifferentialEquations,  CalculusWithJulia, SymPy
end
...
Existen conjuntos de curvas que comparten propiedades particulares, unidas por ecuaciones que incorporan parámetros variables, estos conjuntos son conocidos como familias de curvas. Se mostraran ejemplos ilustrativos de familias de curvas.

Además de estudiar las familias de curvas, también se mostraran las trayectorias ortogonales. Estas son curvas que, en cada punto de intersección con otra curva, tienen tangentes perpendiculares entre sí.
"""

# ╔═╡ 381eb505-54f6-4afc-a3ca-b61c8381a9b5
md"""# Familias de curvas

Una familia de curvas se refiere a un conjunto de curvas que comparten una característica común o están relacionadas por una ecuación general que incluye un parámetro o constante."""
...
begin
	x_values = range(-c, stop=c, length=1000)
	y_values = sqrt.(c^2 .- x_values.^2)
	plot(x_values, y_values, label=false, xlabel="x", ylabel="y", title="Curva x^2 + y^2 = $c ^2", linecolor=:red)
	plot!(x_values, -y_values, label=false, xlabel="", ylabel=" ", linecolor=:red)
end

# ╔═╡ 18bd48f1-651c-4b75-8dfa-c8f5d73811a4
md"""Si derivamos la ecuación anterior con respecto a $x$, se tiene lo siguiente

$2x+2y\frac{dy}{dx}=0,$
esto es

$x+y\frac{dy}{dx}=0,$
esta es la ecuación diferencial de la familia de círculos dada."""
...
let
	if a == 0
    	scatter([0], [0], label=false, xlabel="x", ylabel="y", title="Curva x^2 + y^2 = 2x$a", linecolor=:red)
	elseif a < 0
		x_values = range(2*a, stop=0, length=1000)
    	y_values = sqrt.(2*a .* x_values .- x_values.^2)
    	plot(x_values, y_values, label="x^2 + y^2 = 2x", xlabel="x", ylabel="y", title="Curva x^2 + y^2 = 2x$c", linecolor=:red)
    	plot!(x_values, -y_values, label=false, linecolor=:red)
	elseif a>0
		x_values = range(0, stop=2*a, length=1000)
    	y_values = sqrt.(2*a .* x_values .- x_values.^2)
    	plot(x_values, y_values, label="x^2 + y^2 = 2x", xlabel="x", ylabel="y", title="Curva x^2 + y^2 = 2x", linecolor=:red)
    	plot!(x_values, -y_values, label=false, linecolor=:red)
	end
end

# ╔═╡ 074062c8-ec59-4692-a49a-f81e32db7463
md"""Una **trayectoria ortogonal** es una curva que, en cada punto de intersección con otra curva, tiene una pendiente que es el negativo del recíproco de la pendiente de la curva original en ese mismo punto. Esto implica que las tangentes de ambas curvas son perpendiculares entre sí en los puntos de intersección.  Así, a lo largo de cualquier trayectoria ortogonal, tenemos que 

$\frac{dy}{dx} = \frac{-1}{f(x,y)} \hspace{1.5cm}\text{ o }\hspace{1.5cm} -\frac{dx}{dy} = f(x, y).$
Vemos un par de ejemplos."""

# ╔═╡ 161df47d-7eae-40d2-8337-aec2522ec7a0
md"""**Ejemplo:**

Describamos las trayectorias ortogonales para la familia de curvas dada por $y=\frac{C}{x},$ con $C\neq 0$. 

Note que $xy=C$, si derivamos con respecto a $x$ se tiene que

$x\frac{dy}{dx}+y=0,$
esto es

$\frac{dy}{dx}=-\frac{y}{x},$
así la pendiente de la familia ortogonal es

$\frac{dy}{dx}=\frac{x}{y}.$
Para hallar las trayectorias de esta familia debemos resolver la ecuación diferencial ordinaria."""

# ╔═╡ efd78621-8fb4-4edd-83c3-37930987dffd
md"""Las variables simbólicas que se usarán son las siguientes:"""

# ╔═╡ 4674236d-760b-40d1-8b49-83a76548559e
@syms y() x

# ╔═╡ ba16dbd1-8cc3-4293-9e95-de9e3712bfa0
md"""Resolvamos la primera ecuación. Iniciemos definiendo la ecuación diferencial."""

# ╔═╡ 86939405-879f-4e1d-b989-52cca6b24dd1
begin
	D = Differential(x)
	eqn = D(y)(x) ~ x/y(x)
end

# ╔═╡ 7ff03da0-55e4-4281-9039-bf56f387f3bd
md"""Luego la resolvemos de la siguiente manera"""

# ╔═╡ b5161069-86ea-47cd-9e0d-7ab2f28a1038
solucion = dsolve(eqn)

# ╔═╡ 75ffcc8e-8cf3-499d-86f4-60d35dafe15b
...
let
	x1_values = range(-8, stop=-0.001, length=1000)
	x2_values = range(0.001, stop=8, length=1000)
	f(x) = C/x
	plot(x1_values, f.(x1_values), xlims=(-8, 8), ylims=(-8, 8), label=false, linecolor=:red)
	plot!(x2_values, f.(x2_values), label=false, linecolor=:red)
	if C₁>=0
		x_values = range(-8, stop=8, length=1000)
		y(x) = sqrt(x^2 + C₁)

		plot!(x_values, y.(x_values), label=false, xlabel="x", ylabel="y", title="Curvas y^2 - x^2 = $C₁, xy= $C", linecolor=:blue)
		plot!(x_values, -y.(x_values), label=false, linecolor=:blue)
	else
		x(y) = sqrt(y^2 - C₁)
		y_values = range(-8, stop=8, length=1000)

		plot!(x.(y_values), y_values, label=false, xlabel="x", ylabel="y", title="Curvas y^2 - x^2 = $C₁, xy= $C", linecolor=:blue)
		plot!(-x.(y_values), y_values, label=false, linecolor=:blue)
	end
end

# ╔═╡ 87ce4f40-8452-4902-b47b-57e2b4ef342e
md"""**Ejemplo:**

Encontremos las trayectorias ortogonales de la familia de curvas $x=ky^2,$ donde $k$ es una constante arbitraria.

Derivando la ecuación con respecto a $x$ se tiene que
...
Así, para hallar las trayectorias de esta familia debemos resolver la anterior ecuación diferencial ordinaria."""

# ╔═╡ 7240e2d6-428e-4508-96e9-64cdf8b274b1
md"""Resolvamos la primera ecuación. Iniciemos definiendo la ecuación diferencial."""

# ╔═╡ bc5b0060-b6ff-4d02-af5d-7b66ab9e95b9
eqn1 = D(y)(x) ~ -2*x/y(x)

# ╔═╡ 5af650bd-de8f-4040-ba4d-1f24866aa7c5
md"""Luego la resolvemos de la siguiente manera"""

# ╔═╡ a777d361-4b5a-41d2-a16f-1e1ed8d4b92f
solucion1 = dsolve(eqn1)

# ╔═╡ c2df7563-41d4-480a-bbf3-5fd740edeb5f
...
let
	...
	plot(x.(y_values), y_values, label=false, xlims=(-5, 5), ylims=(-5, 5),xlabel="x", ylabel="y", title="Curvas x = $k y^2, x^2 + y^2 / 2= $C₂", linecolor=:red)

	x_values = range(-sqrt(C₂), stop=sqrt(C₂), length=1000)
	y(x) = sqrt(abs(2 * (C₂ - x^2)))

	plot!(x_values, y.(x_values), label=false, xlabel="x", ylabel="y", linecolor=:blue)
	plot!(x_values, -y.(x_values), label=false, linecolor=:blue)
end
...
manifest_format = "2.0"
project_hash = "8ab3176f75576b0c8ac5166ece11568f2589be7d"
