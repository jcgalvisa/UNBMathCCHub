...
Primero creamos una función que retorne el lado derecho (o forzante) de la EDO de primer orden un su forma canónica. Tambien definimos la condición inicial y el tiempo inicial y final.
"""

# ╔═╡ c73d9679-435c-495f-88f9-8154ec0f5d38
@bind y0 Slider(-5:0.1:10, default=4,show_value=true)

# ╔═╡ 55c84bd1-1b5d-415c-a759-5d20145a2173
begin
	F1(y,α,t)=y-t^2-2*α
#	y0=4
	tspan=(0.0,1.0)
	α=1
	ODE1=ODEProblem(F1,y0,tspan,α)
end

# ╔═╡ 52a56709-607a-4644-9240-df53cde2d908
solution1=solve(ODE1)
#typeof(solution1)

# ╔═╡ c513d916-ae6e-4474-98e0-2212e72ad465
md"""Ahora usamos Plots para desplegar gráficamente la solución. """

# ╔═╡ 279b8f23-9d3f-4d32-bce9-071b72af5d54
plot(solution1,label="Sol1",width=5, color="red")
...
@bind λ Slider(0.5:0.1:4, default=1.5,show_value=true)

# ╔═╡ 5473012d-f309-4f47-b087-b1a99b09ac9c
begin
	RHSheat(u,λ,t)=-λ*u+90+22.5*sin(2*π*t)
	T0=70.0
	tspanheat=(0.0,10.0)
#	λ= 1.5
	heat = ODEProblem(RHSheat,T0,tspanheat,λ)
end

# ╔═╡ e5b827d2-b54a-4d59-97e3-65b5cd137b98
begin
	Temperature=solve(heat)
	plot(Temperature,label="Tem",width=5, color="red")
	time=0.0:0.01:10
	ExternalTemp= 60.0*ones(size(time))+15*sin.(2*π*time)
	plot!(time,ExternalTemp,label="External", color="olive", width=3, legend=:bottomleft)
end

# ╔═╡ 1c2ab61c-602a-402f-b0a6-09ac404ac753
md"""# El modelo de Bertalanffy de crecimiento de tumores
...
begin
	...
	plot(Volume,vars=(0,1),label="Vol",width=5, color="red",legend=:bottomright)
end
...
begin
	...
	ts=(0.0,0.5)
	x0=[1 -1]
	prob = ODEProblem(ld_sistema!,x0,ts,p)
	sol_sistema=solve(prob);
end

# ╔═╡ 7279b82e-8cd8-467b-8844-e09679013af2
md"""Note que en este caso tenemos dos componentes, es decir, dos funciones dependientes del tiempo. Tenemos várias opticiones para desplegar gráficamente la solución. 
...
begin
	plot(sol_sistema,vars=(1,2), width=4)
end
...
@bind rLz Slider(1:0.1:20, default=10)
...
begin
	...
	tspanLz = (0.0,100.0)
	pLz = [10.0,28.0,8/3]
	probLz = ODEProblem(parameterized_lorenz!,u0Lz,tspanLz,pLz)
...
begin
	solLz= solve(probLz)
	plot(solLz, vars=(1,2,3),width=0.3, color="red")
end
...
begin
	plot(solpd,vars=(1,2,3))
#	plot!(solpd,vars=(1,2,4))
end
...
git-tree-sha1 = "afaf97ee668461d836dd00e69528c7759eddc163"
